...
#' @param method The method to use for k-weight optimization. Options include: "ref-sample" (default) or "id.score" 
...
#' @export
map_to <- function(ref, query, lab, method = "ref-sample", range = c(5,100), sample = F, dims = 50){
  if(method == "id.score"){
    if(sample != F){
      set.seed(1984)
    ...
    if(sample %!in% c(50:10000)){ stop("Must enter sample size between 50 and 10,000 when method = \'ref-sample\'")
      ...
      print("Running optimization on query data...")
      
      k_ref <<- scores.list$k[which(scores.list$correct == max(scores.list$correct))]
      ref.scores.list <<- scores.list
      
      #### Query Sample ####
      query <- FindVariableFeatures(query, selection.method = "vst", nfeatures = 2000)
      query <- RunPCA(query, npcs = dims)
      query <- FindNeighbors(query, dims = 1:dims)
      query <- FindClusters(query, resolution = 0.5)
      
      lab <- "seurat_clusters"
      sam.perc <- ncol(ref.list[[2]])/ncol(ref)
      sam.query <- round(sam.perc*ncol(query))
      
      query$split <- 1
      set.seed(1984)
      query$split[sample(1:ncol(query), sam.query, replace = F)] <- 2
      ...
      query.list[[1]] <- Seurat::RunUMAP(query.list[[1]], dims = 1:50, return.model = T) 
      #lab2 <- lab[which(names(lab) %in% colnames(query.list[[1]]))]
      ...
      scores.list <- data.frame(k = 0, correct = 0, score = 0)
      for(k in range[1]:range[2]){
        query.list[[2]] <- Seurat::TransferData(
          anchorset = anchors,
          reference = query.list[[1]],
          query = query.list[[2]],
          refdata = query.list[[1]][[lab]][,1],
          k.weight = k,
          weight.reduction = "pcaproject"
        )
        query.list[[2]] <- Seurat::IntegrateEmbeddings(
          anchorset = anchors,
          reference = query.list[[1]],
          query = query.list[[2]], 
          new.reduction.name = "ref.pca",
        ...
        query.list[[2]] <- Seurat::ProjectUMAP(
          query = query.list[[2]], 
          query.reduction = "ref.pca", 
          reference = query.list[[1]], 
          reference.reduction = "pca", 
        ...
        scores <- c(k, length(which(query.list[[2]]$predicted.id == query.list[[2]][[lab]][,1]))/length(query.list[[2]][[lab]][,1]), mean(query.list[[2]]$predicted.id.score))
        scores.list <- rbind(scores.list, scores)
      }
      print(paste("Optimal k = ", scores.list$k[which(scores.list$correct == max(scores.list$correct))],
                  "; Mean score = ", scores.list$score[which(scores.list$correct == max(scores.list$correct))], sep = ""))
      print("Running map_to with optimized k on query data...")
      
      k_query <<- scores.list$k[which(scores.list$correct == max(scores.list$correct))]
      query.scores.list <<- scores.list
    }
    
  }
  
  #### Run Final ####
  k <- round(mean(k_ref, k_query))
